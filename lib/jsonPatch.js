...
function patchInPlace(changes, x, options) {
	...
	for (var i = 0; i < changes.length; ++i) {
		...
		// At the mo this is just making sure we mark subproperties of a Mixed (e.g. any)
		// schema as modified when any property within it changes.
		// To see if it is a mongoose object just check for the properties we'd expect
		const mongoosePath = toMongoosePath(p.path);
		const mongooseSchemaType =
			x.schema &&
			typeof x.set === 'function' &&
			typeof x.markModified === 'function' &&
			x.schema.path(mongoosePath);
		const isMongooseMixedSubproperty =
			mongooseSchemaType &&
			mongooseSchemaType.instance === 'Mixed' &&
			mongoosePath !== mongooseSchemaType.path;

		// If this isn't a Mixed mongoose object subproperty then we want to set properties to `undefined`
		// as our delete (so mongoose will pick them up). If it is a Mixed object subprop then we
		...
		x = patch.apply(x, p, { ...options, deleteOnRemove: isMongooseMixedSubproperty });

		if (p.op !== 'test' && isMongooseMixedSubproperty) {
