...
function applyReplace(x, change, options) {
	...
	if (Array.isArray(target)) {
		let idx = parseArrayIndex(pointer.key);

		// If we're dealing with a mongoose array then we
		// need to call `.set` otherwise we don't get type casting.
		// Use duck-typing to detect this
		if (typeof target.set === 'function' && target.isMongooseArray) {
...
function applyRemove(x, change, options) {
	...
	_remove(pointer, options.deleteOnRemove);
	return x;
}

function _remove(pointer, performDelete) {
	var target = pointer.target;
